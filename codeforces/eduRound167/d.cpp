...
template<class T> struct SegTree { // cmb(ID,b) = b
	const T ID{MX}; T cmb(T a, T b) { return min(a, b); } 
	int n; vector<T> seg;
...
int dfs(int val) {
  ...
  dbg("ok");
  pi b;
  b.f = seg.query(1, min(val, MX-1));
  if(b.f == MX) return 0;
  b.s = mep[b.f];
  assert(b.f != 0);

  int res = (val-b.s)/b.f+1;
  int next = val-res*b.f;
  res += dfs(next);
  return memo[val] = res;
}
...
void solve() {
  ...
  vi C(M); F0R(i, M) cin>>C[i];
  
  seg.init(MX);

  F0R(i, N) {
    if(mep.count(A[i]-B[i]) == 0) mep[A[i]-B[i]] = A[i];
    else ckmin(mep[A[i]-B[i]], A[i]);
    if(seg.query(A[i], A[i]) > A[i]-B[i]) {
      seg.upd(A[i], A[i]-B[i]);
    }
  }
